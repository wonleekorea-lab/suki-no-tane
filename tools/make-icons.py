...
DOT_R = 0.150
RINGS = [
    (0.262, 0.020, 0.62),   # 半径, 線幅, 濃さ（外へ行くほど細く淡く＝広がって消える）
    (0.378, 0.013, 0.28),
]
...
if __name__ == "__main__":
    for size in (180, 192, 512):
        ...
        # 端末に残った古いHTML／マニフェストは旧名を指している。
        # 旧名を消すと参照が404になり、iOSはアイコンを1枚も読めず
        # タイトルの1文字を黒地に置いた代替を出す。旧名にも同じ絵を置いておく。
        write_png(size, "%s/icon-%d.png" % (OUT, size))
